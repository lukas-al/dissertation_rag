...
def construct_graph(
    ...
    **kwargs,
    ...
    for doc in tqdm.tqdm(embedded_index, desc='Constructing Graph'):
        ...
        retrieved_docs = retrieval_algorithm(np.array(doc.embedding), embedded_index, **kwargs)
...
def simple_stats(graph: nx.Graph) -> Dict[AnyStr, Any]:
    ...
    stats['average_degree'] = sum(dict(graph.degree()).values()) / float(nx.number_of_nodes(graph))
...
def attribute_stats(graph: nx.Graph) -> Dict[AnyStr, Any]:
    ...
    stats['source_assortativity'] = nx.attribute_assortativity_coefficient(graph, 'file_name')
    stats['page_assortativity'] = nx.attribute_assortativity_coefficient(graph, 'page_label')
    stats['date_assortativity'] = nx.numeric_assortativity_coefficient(graph, 'date_num')
